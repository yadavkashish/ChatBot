...
*/
var QdrantVectorStore = class QdrantVectorStore extends _langchain_core_vectorstores.VectorStore {
	get lc_secrets() {
		return {
	...
	constructor(embeddings, args) {
		...
		const url = args.url ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("QDRANT_URL");
		const apiKey = args.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("QDRANT_API_KEY");
		if (!args.client && !url) throw new Error("Qdrant client or url address must be set.");
		this.client = args.client || new _qdrant_js_client_rest.QdrantClient({
			url,
			apiKey
		});
	...
	*/
	async addVectors(vectors, documents, documentOptions) {
		...
		const points = vectors.map((embedding, idx) => ({
			id: documents[idx].id ?? documentOptions?.ids?.[idx] ?? (0, _langchain_core_utils_uuid.v4)(),
			vector: embedding,
			payload: {
		...
		try {
			...
			throw /* @__PURE__ */ new Error(`${e?.status ?? "Undefined error code"} ${e?.message}: ${e?.data?.status?.error}`);
		}
	}
	/**
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return (await this.client.query(this.collectionName, {
			...
			with_vector: false
		})).points.map((res) => [new _langchain_core_documents.Document({
			id: res.id,
	...
	*/
	async ensureCollection() {
		if (!(await this.client.getCollections()).collections.map((collection) => collection.name).includes(this.collectionName)) {
